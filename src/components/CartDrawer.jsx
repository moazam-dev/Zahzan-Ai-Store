...
export default function CartDrawer() {
  const { cartItems, isCartOpen, closeCart, removeFromCart, updateQuantity, cartCount, cartTotal, clearCart } = useCart()
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-hidden" role="dialog" aria-modal="true">
      ...
      {/* ========================================================================= */}
      {/* RIGHT-SIDE CART DRAWER (30vw Desktop / 100vw Fullscreen Mobile) */}
      {/* ========================================================================= */}
      <aside
      ...
      >
        ...
        {/* ----------------------------------------------------------------------- */}
        {/* CART HEADER: YOUR BAG + ITEM COUNT + ELEGANT CLOSE BUTTON */}
        {/* ----------------------------------------------------------------------- */}
        ...
        {cartItems.length > 0 && (
          <div className="sticky bottom-0 bg-[#faf8f5] border-t border-[#e8e4dc] px-6 py-5 sm:px-8 space-y-4">
            ...
            <button
              type="button"
              onClick={() => setIsCheckoutSubmitted(true)}
      ...
      {isCheckoutSubmitted && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-[#faf8f5] border border-[#e8e4dc] rounded-xs p-6 sm:p-8 max-w-md w-full relative shadow-2xl text-center space-y-4">
            <span className="text-[10px] font-sans font-medium uppercase tracking-[0.35em] text-[#5a5e4b] block">
              THANK YOU FOR YOUR SELECTION
            </span>
            <h3 className="font-serif text-3xl text-[#1c1b18]">Order Confirmed</h3>
            <p className="text-xs font-sans text-[#706c64] leading-relaxed">
              Your order total of <span className="font-medium text-[#1c1b18]">PKR {cartTotal.toLocaleString()}</span> has been processed. A confirmation email has been dispatched.
            </p>
            <div className="pt-4">
              <button
                type="button"
                ...
                className="w-full bg-[#1c1b18] text-white text-xs font-sans uppercase tracking-[0.25em] py-3 hover:bg-[#5a5e4b] transition-colors cursor-pointer"
              >
                Return to Shop
              </button>
            </div>

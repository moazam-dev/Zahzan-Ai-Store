...
export default function Account() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setUser(data.user)
        setProfileData({ name: data.user.name || '', phone: data.user.phone || '' })
        showFeedback('success', data.message || 'Account created successfully. Welcome to ZAHZAN!')
        fetchUserProfile(data.token)
      } else {
  ...
  const handleFacebookLogin = async (fbProfile = null) => {
    setStatusMsg(null)
    try {
      const payload = fbProfile || {
        facebookId: `fb_${Date.now()}`,
        email: `facebook.user.${Date.now().toString().slice(-4)}@facebook.com`,
      ...
      if (data.success) {
        ...
        setUser(data.user)
        setProfileData({ name: data.user.name || '', phone: data.user.phone || '' })
        showFeedback('success', 'Logged in via Facebook successfully.')
        fetchUserProfile(data.token)
      } else {
        showFeedback('error', data.message || 'Facebook login failed.')
      }
    } catch (err) {
      showFeedback('error', 'Unable to connect to server for Facebook authentication.')
    }
  }
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3 pt-2">
                  <div className="relative flex py-1 items-center">
                    ...
                    <span className="flex-shrink mx-3 text-[10px] font-sans uppercase tracking-widest text-[#706c64]">OR CONTINUE WITH</span>
                    <div className="flex-grow border-t border-[#e8e4dc]"></div>
                  </div>
            ...
            {mode === 'register' && (
              <div className="space-y-6">
                ...
                <div className="space-y-3 pt-2">
                  <div className="relative flex py-1 items-center">
                    <div className="flex-grow border-t border-[#e8e4dc]"></div>
                    <span className="flex-shrink mx-3 text-[10px] font-sans uppercase tracking-widest text-[#706c64]">OR REGISTER WITH</span>
                    <div className="flex-grow border-t border-[#e8e4dc]"></div>
                  </div>

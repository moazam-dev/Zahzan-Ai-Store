...
export async function checkRateLimit(request, config) {
  ...
  return checkKeyedRateLimit(`${config.id}:${ip}`, config);
}

/**
 * The same limiter, but against a caller-supplied key instead of the client
 * IP. Added for Virtual Try-On (ruling P5), which must limit per ACCOUNT, not
 * per address: AI generation costs real money per call, and an IP-only limit
 * is both too loose (one account, many addresses) and too tight (a shared
 * office or mobile CGNAT address is many accounts).
 *
 * Shares check_rate_limit() and the fail-open behaviour documented below, so
 * there is exactly one rate-limiting implementation in the codebase.
 *
 * @param {string} key already-qualified, e.g. `tryon-user:<uuid>`
 * @param {{ max: number, windowSeconds: number, message: string }} config
 */
export async function checkKeyedRateLimit(key, config) {

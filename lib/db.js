...
// against the actual `next build` output (not just reasoned about): 20
// distinct server chunks each inline their own copy of this file, INCLUDING
// their own `new Pool(...)` call -- so a plain `let pgPool` gave every one
// of those chunks its own independent Pool, each defaulting to `pg`'s own
// `max: 10`, for up to ~200 real connections against Supabase's pooler from
// a single Node process under trivial concurrent load. This was found at
// build-output level (grepping the compiled server chunks for `new Pool`),
// not inferred from source alone -- a future reader should not "simplify"
// this back to a module-scope `let` on the theory that a Pool is somehow
// different from the PGlite singleton; empirically, for Turbopack's
// per-entry-point bundling, it is not. `globalThis` is the one thing every
// separately-bundled entry point genuinely shares within one OS process, so
// storing the Pool there (exactly the pattern already used for PGlite two
// keys below) is what actually makes "one Pool per process" true.

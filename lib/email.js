...
//      function may freeze before the request completes. `dispatch()`
//      below is the seam Tasks 8-13's route handlers use instead of the
//      old fire-and-forget `Promise.all([...]).catch(...)` pattern --
//      `await dispatch(sendX(...))` is always safe to await regardless of
//      mode, and in test mode (ZAHZAN_EMAIL_SYNC=1) it deterministically
//      waits for the real result instead of racing the test.
 ...
 *        senders above.
 * @returns {Promise<boolean|undefined>} in sync mode
 *        (ZAHZAN_EMAIL_SYNC=1), awaits and returns the sender's real
 *        true/false result. Otherwise, fires the promise off without
 *        blocking the caller (a `.catch()` logs a failure -- defensive
 *        only, since every sender above already catches its own errors
 *        internally and resolves to false rather than rejecting) and
 *        resolves immediately with `undefined`.
 */
export async function dispatch(promise) {
  if (process.env.ZAHZAN_EMAIL_SYNC === '1') {
    return promise;
  }

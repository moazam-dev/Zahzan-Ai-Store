...
/**
 * Fields added AFTER the migration by 0004_product_details.sql -- NOT part of
 * server/models/Product.js, so they are kept out of PRODUCT_TRIM_FIELDS above
 * to keep that list diffable against the old schema. They are trimmed for the
 * same reason everything else here is: an admin who pastes a value with a
 * trailing space should not have that space rendered on the product page.
 */
export const PRODUCT_EXPANSION_TRIM_FIELDS = ['modelHeight', 'modelSize', 'fitNote'];
...
/**
 * server/models/OrderItem.js -- `size`/`color` fields (both `trim: true`).
 * Request bodies name these `selectedSize`/`selectedColor`; only the
 * buy-now path (app/api/orders/route.js) needs to trim them explicitly --
 * the cart-checkout path's items come from `cart_items`, already trimmed at
 * cart-insert time (see CART_ITEM_TRIM_FIELDS / app/api/cart/items/route.js).
 */
export const ORDER_ITEM_TRIM_FIELDS = ['selectedSize', 'selectedColor'];
...
export function trimProductPayload(body) {
  ...
  for (const field of PRODUCT_EXPANSION_TRIM_FIELDS) {
    if (result[field] !== undefined) result[field] = trimIfString(result[field]);
  }

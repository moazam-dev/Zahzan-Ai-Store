...
export function serializeProduct(row) {
  ...
  const out = {
    ...
    gallery: row.gallery ?? [],
    stock: row.stock,
    // Per-size inventory (0004_product_details.sql). Always present, never
    // absent, because the column is `not null default '{}'` -- the same rule
    // the fields above follow. An empty object means "this product is not
    // size-tracked" and every consumer falls back to `stock`, which is why
    // this is safe to emit for products that predate the migration.
  ...
  // 0004_product_details.sql. Nullable columns with no default, so they follow
  // the same "genuinely absent when unset" rule as the block above rather than
  // appearing as null. modelHeight/modelSize are the structured inputs the API
  // composes modelInfo from; fitNote is the sentence views/Product.jsx used to
  // hardcode.
  if (row.model_height != null) out.modelHeight = row.model_height;
  if (row.model_size != null) out.modelSize = row.model_size;
  if (row.fit_note != null) out.fitNote = row.fit_note;
  return out;
}
...
/**
 * M1 fix (final whole-branch review): app/api/newsletter/subscribe/route.js's
 * new-subscriber success branch hand-built its response object directly
 * (`{ id, email, status, subscribedAt: newSubscriber.subscribed_at }`)
 * instead of going through this module, passing `subscribed_at` through as a
 * raw Date rather than an ISO string via `toIso()` -- the only timestamp in
 * the codebase that reached a client without passing through lib/serialize.js.
 * It happened to produce byte-identical JSON today (`JSON.stringify` calls
 * `Date.prototype.toJSON`, which is `toISOString()`), but bypassed the
 * serializer's explicit conversion. This is a narrow variant, precedented by
 * serializeOrderSummaryForPayment/serializeDashboardRecentCustomer above:
 * deliberately NOT serializeNewsletterSubscriber's shape (no `_id`, `source`,
 * `unsubscribedAt`, `createdAt`, `updatedAt`, or `userId`) -- the source's
 * hand-built object only ever carried these four keys, confirmed against
 * tools/golden/051-newsletter.subscribe.json, and GC1 forbids adding keys
 * the old API never emitted.
 */
export function serializeNewsletterSubscribeResult(row) {
  if (row == null) return null;
  return {
    id: row.id,
    email: row.email,

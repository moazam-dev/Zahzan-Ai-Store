...
// both `_id` and `id`) and GC3 (snake_case columns -> camelCase JSON) --
// no route handler is allowed to hand-build a response entity itself.
//
...
//    that one field.
//
// A third thing, NOT found empirically but reasoned from GC2's binding
// text: `addresses` (and, by the same "no custom toJSON transform"
// pattern, story_submissions/tryon_jobs, which no live endpoint has ever
// produced -- see the Task 4 report) currently serialize with `_id` only,
// no `id`, confirmed by tools/golden/022-users.address-create.json. GC2
// ("Every serialized entity carries both _id and id ... Omitting _id
// silently breaks the saved address list") is a Global Constraint that
// binds regardless of a task's own text, and its one documented exemption
// (Ruling C8, progress.md) is `serializeAuthUser` alone. So
// serializeAddress/serializeStory/serializeTryOnJob deliberately ADD an
// `id` the current raw Mongoose output does not have. This is intentional
// GC2 compliance, not a parity bug.
 ...
 */
function serializePopulatedUserSummary(row) {
 ...
 *        entirely omits `payment` from the output altogether. Confirmed
 *        against tools/golden/037-orders.create-cod-1.json (`"payment":
 *        null` IS present there -- createOrder always attaches a payment
 *        key, null for COD) vs. tools/golden/039-orders.my-orders.json
 *        (no `payment` key on any order at all).
 */
...
export function serializeAddress(row) {
  if (row == null) return null;
  return {
    // GC2 addition: today's raw Address output (no custom toJSON
    // transform at all) has `_id` but NOT `id` -- confirmed against
    // tools/golden/022-users.address-create.json. GC2 is a Global
    // Constraint and its only documented exemption is serializeAuthUser
    // (Ruling C8); addresses are explicitly named in GC2's own text as one
    // of the entities it exists to protect. See the header comment.
    _id: row.id,
    userId: row.user_id,

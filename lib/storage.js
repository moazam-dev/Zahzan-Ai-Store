...
// ---------------------------------------------------------------------------
// Virtual Try-On images (ruling P5)
// ---------------------------------------------------------------------------
//
// A THIRD bucket, `tryon-images`, PRIVATE -- same design as payment-proofs and
// for a stronger reason: these are photographs of customers' faces and bodies.
// The DB column stores the storage PATH, never a URL, and every read signs a
// fresh short-lived URL. A signed URL persisted into the column would 403 the
// moment it expired.
//
// Retention is enforced elsewhere (tryon_jobs.expires_at +
// tools/purge-tryon.mjs); this module only provides put/sign/delete.

const TRYON_BUCKET = 'tryon-images';

/**
 * Stores a try-on image (input photograph or generated result).
 *
 * @param {Buffer} buffer
 * @param {string} contentType e.g. 'image/png'
 * @param {string} kind 'input' | 'output' -- only a path prefix, for humans
 *        reading the bucket
 * @returns {Promise<{ path: string }>} the storage PATH to persist. Deliberately
 *        NOT a URL: see the header comment.
 */
export async function uploadTryOnImage(buffer, contentType, kind = 'output') {
  const extension = contentType === 'image/jpeg' ? 'jpg' : contentType === 'image/webp' ? 'webp' : 'png';
  const filename = `${kind}.${extension}`;

  if (getDriverName() === 'memory') {
    return { path: memoryUpload(`tryon_${kind}`, filename, buffer, contentType) };
  }

  const path = storagePath(`tryon/${kind}`, filename);
  await supabaseUpload(TRYON_BUCKET, path, buffer, contentType);
  return { path };
}

/**
 * Signs a short-lived URL for a stored try-on image.
 * Returns '' for an empty path so callers can pass a not-yet-populated
 * `output_image` straight through without branching.
 */
export async function signTryOnUrl(path) {
  if (!path) return '';
  if (getDriverName() === 'memory') {
    return `memory://tryon-images/${path}`;
  }
  return supabaseSignedUrl(TRYON_BUCKET, path);
}

/**
 * Deletes a stored try-on image. Never throws: this is called from retention
 * purges and from DELETE /api/try-on/:id, and a missing object (already gone,
 * or never written because generation failed) must not fail the whole
 * operation or strand the database row.
 *
 * @returns {Promise<boolean>} whether the object was actually removed
 */
export async function deleteTryOnImage(path) {
  if (!path) return false;
  try {
    if (getDriverName() === 'memory') {
      return memoryStore.delete(path);
    }
    await supabaseDelete(TRYON_BUCKET, path);
    return true;
  } catch (err) {
    console.warn('deleteTryOnImage: could not remove object:', err?.message || err);
    return false;
  }
}
